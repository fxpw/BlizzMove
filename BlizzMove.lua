...
local function OnMouseUp(self, ...)
	...
	if IsControlKeyDown() then
		...
		if settings then
			...
			Print("Frame: ",frameToMove:GetName()," will be saved.")
			_tempDBBlizzMove[frameToMove:GetName()] = {}
			settings = _tempDBBlizzMove[frameToMove:GetName()]
			settings.save = true
			settings.point, settings.relativeTo, settings.relativePoint, settings.xOfs, settings.yOfs = frameToMove:GetPoint()
...
local function SetMoveHandler(frameToMove, handler)
	...
	local settings = _tempDBBlizzMove[frameToMove:GetName()]
	if not settings then
		settings = defaultDB[frameToMove:GetName()] or {}
		_tempDBBlizzMove[frameToMove:GetName()] = settings
	end
	frameToMove.settings = settings
...
local function resetDB()
	for k, _ in pairs(_tempDBBlizzMove) do
		local f = _G[k]
...
local function OnEvent(self, event, arg1, arg2)
	...
	if event == "PLAYER_ENTERING_WORLD" then
		...
		_tempDBBlizzMove = BlizzMoveDB or defaultDB
		BlizzMoveDB = _tempDBBlizzMove
		...
		SetMoveHandler(MacroFrame,MacroPopupFrame)
		SetMoveHandler(MacroPopupFrame)
		SetMoveHandler(EncounterJournal)
		SetMoveHandler(PVPUIFrame)
		SetMoveHandler(GuildFrame)
		SetMoveHandler(StoreFrame)
		SetMoveHandler(CollectionsJournal)
		SetMoveHandler(InspectEquip_InfoWindow)
		SetMoveHandler(FishingOptionsFrame)
		...
		SetMoveHandler(BattlePassFrame)
